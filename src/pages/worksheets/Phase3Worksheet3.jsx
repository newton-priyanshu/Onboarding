...
export default function Phase3Worksheet3() {
  ...
  const {
    ...
    defaultData: {
      ...
      blueprintAssessmentType: '',
      blueprintDifficultyDistribution: '',
      blueprintFeedbackLoop: '',
    },
  ...
  const uBloom = (i, f, v) => setData(p => { const arr = [...p.bloomGrid]; arr[i] = { ...arr[i], [f]: v }; return { ...p, bloomGrid: arr }; });

  if (isBuddyApproved) return <BuddyApprovedView msg="Your Assessment Blueprint has been approved by your buddy." path="/phase-3" />;
  if (isApproved) return <ApprovedView msg="Your Assessment Blueprint worksheet has been reviewed and approved." path="/phase-3" reviewerName={data._savedReviewerName} date={data._savedReviewedAt} />;
  if (isSubmitted) return <SubmittedView msg="Assessment Blueprint submitted for review." path="/phase-3" />;
  ...
  return (
    <div className="lux-section">
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '0 1rem' }}>
        ...
        <WorksheetHeader
          icon={CheckSquare} title="Assessment Blueprint & Bloom's Taxonomy Grid"
          subtitle="Design assessments that measure learning at every cognitive level" saveStatus={saveStatus}
        />
        ...
        <form onSubmit={e => e.preventDefault()} style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <WorksheetSection title="Assessment Design Decisions" subtitle="Explain the rationale behind your assessment structure.">
            <FieldGroup label="What type of assessment are you designing (quiz, assignment, exam, project) and what are its primary learning objectives?">
              <textarea className="lux-textarea" rows={2} value={data.blueprintAssessmentType || ''}
                onChange={e => updateField('blueprintAssessmentType', e.target.value)} placeholder="Describe the assessment and what it aims to measure..." />
            </FieldGroup>
            <FieldGroup label="How is difficulty distributed across questions — and how do you ensure both strong and struggling students are fairly assessed?">
              <textarea className="lux-textarea" rows={2} value={data.blueprintDifficultyDistribution || ''}
                onChange={e => updateField('blueprintDifficultyDistribution', e.target.value)} placeholder="e.g. 30% recall, 40% application, 30% analysis..." />
            </FieldGroup>
            <FieldGroup label="How will you use assessment results to inform your subsequent teaching (closing the feedback loop)?">
              <textarea className="lux-textarea" rows={2} value={data.blueprintFeedbackLoop || ''}
                onChange={e => updateField('blueprintFeedbackLoop', e.target.value)} placeholder="How will assessment data shape your next lessons?" />
            </FieldGroup>
          </WorksheetSection>
