...
export default function Phase1Worksheet3() {
  ...
  const {
    ...
    requiredFields: [{ key: 'employeeName', label: 'Full Name' }, { key: 'culturePhilosophy', label: 'Teaching Philosophy reflection' }],
      ...
      <div style={{ maxWidth: '720px', margin: '0 auto', padding: '0 1rem' }}>
        ...
        <form onSubmit={e => e.preventDefault()} style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <WorksheetSection title="Section A: Culture Understanding" subtitle="The shift from industry to teaching requires a deliberate mindset change.">
            <FieldGroup label="Describe Newton School's teaching philosophy in your own words (min. 50 words):" required><textarea className="lux-textarea" rows={3} value={data.culturePhilosophy} onChange={e => updateField('culturePhilosophy', e.target.value)} /></FieldGroup>
            <FieldGroup label="How does classroom communication differ from communication in your previous industry role?"><textarea className="lux-textarea" rows={2} value={data.cultureIndustryDiff} onChange={e => updateField('cultureIndustryDiff', e.target.value)} /></FieldGroup>
            <FieldGroup label="What does 'psychological safety in the classroom' mean to you, and how would you cultivate it?"><textarea className="lux-textarea" rows={2} value={data.culturePsychSafety} onChange={e => updateField('culturePsychSafety', e.target.value)} /></FieldGroup>
          </WorksheetSection>
          <WorksheetSection title="Section B: University Partnership Model">
            <FieldGroup label="Describe the partnership structure between Newton School and its affiliated universities."><textarea className="lux-textarea" rows={2} value={data.partnerStructure} onChange={e => updateField('partnerStructure', e.target.value)} /></FieldGroup>
            <FieldGroup label="How is the academic semester structured — key dates, milestones, and exam windows?"><textarea className="lux-textarea" rows={2} value={data.semesterStructure} onChange={e => updateField('semesterStructure', e.target.value)} /></FieldGroup>
            <FieldGroup label="What do students at your institution primarily expect from their instructors?"><textarea className="lux-textarea" rows={2} value={data.studentExpectations} onChange={e => updateField('studentExpectations', e.target.value)} /></FieldGroup>
          </WorksheetSection>
