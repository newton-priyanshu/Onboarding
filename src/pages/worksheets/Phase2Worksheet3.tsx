  ...
  return (
    ...
    >
      {({ data, updateField, setData }) => {
        const uE = (i: number, f: string, v: string | boolean) => setData(p => { const arr = [...p.entries]; arr[i] = { ...arr[i], [f]: v }; return { ...p, entries: arr }; });
        ...
        const removeEntry = (i: number) => { if (data.entries.length > 1) setData(p => ({ ...p, entries: p.entries.filter((_: unknown, idx: number) => idx !== i) })); };
        return (
          <>
            ...
            <WorksheetSection title="Content Creation Tracker" subtitle="All content must be reviewed by mentor or Faculty Lead before release.">
              ...
              <div className="ws-scroll-x">
                <div className="ws-matrix-row" style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.5fr 0.8fr 0.5fr 1fr 0.5fr', gap: '6px', padding: '6px 0', borderBottom: '1px solid var(--color-charcoal)' }}>
                ...
                {(data.entries as Array<Record<string, unknown>>).map((e, i) => (
                  <div key={i} style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                    <div className="ws-matrix-row" style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.5fr 0.8fr 0.5fr 1fr 0.5fr', gap: '6px', flex: 1 }}>
                      <label htmlFor={`ce-type-${i}`} className="ws-sr-only">Type (row {i + 1})</label>
                      <input id={`ce-type-${i}`} className="lux-input" placeholder="Type" value={e.type as string} onChange={ev => uE(i, 'type', ev.target.value)} />
                      <label htmlFor={`ce-topic-${i}`} className="ws-sr-only">Topic (row {i + 1})</label>
                      <input id={`ce-topic-${i}`} className="lux-input" placeholder="Topic" value={e.title as string} onChange={ev => uE(i, 'title', ev.target.value)} />
                      <label htmlFor={`ce-date-${i}`} className="ws-sr-only">Date (row {i + 1})</label>
                      <input id={`ce-date-${i}`} className="lux-input" type="date" value={e.date as string} onChange={ev => uE(i, 'date', ev.target.value)} />
                      <label htmlFor={`ce-submitted-${i}`} className="ws-sr-only">Submitted (row {i + 1})</label>
                      <input id={`ce-submitted-${i}`} type="checkbox" checked={e.submitted as boolean} onChange={ev => uE(i, 'submitted', ev.target.checked)} style={{ accentColor: 'var(--color-charcoal)', justifySelf: 'center' }} />
                      <label htmlFor={`ce-reviewer-${i}`} className="ws-sr-only">Reviewer (row {i + 1})</label>
                      <input id={`ce-reviewer-${i}`} className="lux-input" placeholder="Name" value={e.reviewer as string} onChange={ev => uE(i, 'reviewer', ev.target.value)} />
                      <label htmlFor={`ce-approved-${i}`} className="ws-sr-only">Approved (row {i + 1})</label>
                      <input id={`ce-approved-${i}`} type="checkbox" checked={e.approved as boolean} onChange={ev => uE(i, 'approved', ev.target.checked)} style={{ accentColor: 'var(--color-charcoal)', justifySelf: 'center' }} />
                    </div>
                    {data.entries.length > 1 && <button type="button" onClick={() => removeEntry(i)} aria-label="Remove entry" className="ws-star-btn" style={{ background: 'none', border: 'none', color: t.error, cursor: 'pointer' }}><Trash2 size={14} strokeWidth={1.5} /></button>}
                  </div>
                ))}
              </div>
            </WorksheetSection>
            ...
            <WorksheetSection title="Reflection"><FieldGroup label="How did content creation improve your understanding of the subject?" id="reflection"><textarea id="reflection" className="lux-textarea" rows={3} value={data.reflection} onChange={e => updateField('reflection', e.target.value)} /></FieldGroup></WorksheetSection>
