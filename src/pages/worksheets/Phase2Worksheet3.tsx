              ...
              {(data.entries as Array<Record<string, unknown>>).map((e, i) => (
                <div key={i} style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.5fr 0.8fr 0.5fr 1fr 0.5fr', gap: '6px', flex: 1 }}>
                    <input className="lux-input" placeholder="Type" value={e.type as string} onChange={ev => uE(i, 'type', ev.target.value)} />
                    <input className="lux-input" placeholder="Topic" value={e.title as string} onChange={ev => uE(i, 'title', ev.target.value)} />
                    <input className="lux-input" type="date" value={e.date as string} onChange={ev => uE(i, 'date', ev.target.value)} />
                    <input type="checkbox" checked={e.submitted as boolean} onChange={ev => uE(i, 'submitted', ev.target.checked)} style={{ accentColor: 'var(--color-charcoal)', justifySelf: 'center' }} />
                    <input className="lux-input" placeholder="Name" value={e.reviewer as string} onChange={ev => uE(i, 'reviewer', ev.target.value)} />
                    <input type="checkbox" checked={e.approved as boolean} onChange={ev => uE(i, 'approved', ev.target.checked)} style={{ accentColor: 'var(--color-charcoal)', justifySelf: 'center' }} />
                  </div>
