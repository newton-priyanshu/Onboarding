        ...
        <>
          ...
          <WorksheetSection title="Framework Application" subtitle="Document which pedagogical frameworks you've experimented with and how they landed.">
            <FieldGroup label="Which pedagogical frameworks or teaching strategies have you consciously applied this phase? (e.g., flipped classroom, think-pair-share, scaffolded instruction, peer instruction, problem-based learning)" required>
              <textarea className="lux-textarea" rows={2} value={data.frameworksApplied || ''} onChange={e => updateField('frameworksApplied', e.target.value)} placeholder="List the frameworks and briefly describe how you adapted them to your context..." />
            </FieldGroup>
            <FieldGroup label="Describe one specific instance where you used an active-learning technique. What was the student response, and what did you learn from it?">
              <textarea className="lux-textarea" rows={2} value={data.activeLearningExample || ''} onChange={e => updateField('activeLearningExample', e.target.value)} placeholder="What did you try, how did students react, and what would you do differently next time?" />
            </FieldGroup>
            <FieldGroup label="Where have you noticed the biggest gap between theory and practice — and how did you bridge it in the moment?">
              <textarea className="lux-textarea" rows={2} value={data.theoryPracticeGap || ''} onChange={e => updateField('theoryPracticeGap', e.target.value)} placeholder="Did a well-planned technique fall flat? How did you adapt?" />
            </FieldGroup>
          </WorksheetSection>
          <WorksheetSection title="Growth as an Educator" subtitle="Track your teaching evolution over Phase 3.">
            <FieldGroup label="What specific teaching behaviour have you iterated on most this phase — and what changed as a result?">
              <textarea className="lux-textarea" rows={2} value={data.iterationNotes || ''} onChange={e => updateField('iterationNotes', e.target.value)} placeholder="e.g. I started using exit tickets — now I can gauge understanding before the next class..." />
            </FieldGroup>
            <FieldGroup label="How has your personal teaching philosophy evolved since you started the onboarding program?">
              <textarea className="lux-textarea" rows={2} value={data.frameworkGrowth || ''} onChange={e => updateField('frameworkGrowth', e.target.value)} placeholder="What do you now believe about teaching that you didn't 90 days ago?" />
            </FieldGroup>
          </WorksheetSection>
        </>
      )}
