      ...
      {({ data, updateField }) => (
        <>
          <WorksheetSection title="Session Details">
            ...
            <FieldGroup label="Session type" id="sessionType">
              <select id="sessionType" className="lux-select" value={data.sessionType as string} onChange={e => updateField('sessionType', e.target.value)}>
            ...
            <FieldGroup label="Date" id="date"><input id="date" type="date" className="lux-input" value={data.date as string} onChange={e => updateField('date', e.target.value)} /></FieldGroup>
            <FieldGroup label="Scenarios encountered / roleplayed" id="scenarios">
              <textarea id="scenarios" className="lux-textarea" rows={3} value={data.scenarios as string} onChange={e => updateField('scenarios', e.target.value)}
                placeholder="e.g. Late arrival, phone ringing, 'this is basic' comment" />
            </FieldGroup>
          </WorksheetSection>
          <WorksheetSection title="Feedback & Reflection">
            <FieldGroup label="Observer feedback received" id="observerFeedback">
              <textarea id="observerFeedback" className="lux-textarea" rows={3} value={data.observerFeedback as string} onChange={e => updateField('observerFeedback', e.target.value)} />
            </FieldGroup>
            <FieldGroup label="Self-reflection" id="selfReflection">
              <textarea id="selfReflection" className="lux-textarea" rows={3} value={data.selfReflection as string} onChange={e => updateField('selfReflection', e.target.value)} />
            </FieldGroup>
          </WorksheetSection>
        </>
      )}
